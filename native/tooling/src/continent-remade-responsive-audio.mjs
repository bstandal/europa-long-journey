...
const sourceRoot = path.join(audioRoot, "continent-remade-responsive-v1");
const cacheRoot = path.join(audioRoot, "cache/continent-remade-responsive-v1");
...
const workObjectPath = path.join(sourceRoot, "continent-remade-responsive-work-object.json");
...
const activeContinentRemadeHaptics = ["drag", "break", "seal"];
const packageRoot = "audio/first-farmers/continent-remade-responsive-v1";
...
function validateSoundscapeSource(source, scoreSource) {
  ...
  assert(JSON.stringify(source.mixGroups) === JSON.stringify({
    soundscape: ["seasonal-ground", "settlement-hearths", "field-work"],
...
async function productionInputs() {
  return Promise.all([
    inputRecord("continent-remade-responsive-score-source", scoreSourcePath),
    inputRecord("continent-remade-responsive-soundscape-source", soundscapeSourcePath),
    ...
    inputRecord("continent-remade-responsive-renderer", modulePath),
...
export async function validateContinentRemadeResponsiveAudioSources() {
  const { scoreSource, soundscapeSource, draft } = await readSources();
  const transformation = findBeat(draft, "beat-first-farmers-continent-remade");
  assert(transformation?.interaction?.grammar === "transform", "Continent Remade source lost its Transform mechanism");
  return {
...
async function buildWorkObject({ scoreSource, soundscapeSource, draft, inputs, outputs, metrics }) {
  const transformation = findBeat(draft, "beat-first-farmers-continent-remade");
  const consequence = findBeat(draft, "beat-first-farmers-before-steppe");
  assert(transformation && consequence, "Continent Remade manuscript beats are missing");
  assert(transformation.interaction?.id === "interaction-first-farmers-a-continent-remade", "Continent Remade interaction ID drifted");
  assert(transformation.interaction.grammar === "transform", "Continent Remade interaction grammar drifted");
  assert(JSON.stringify(transformation.interaction.haptics) === JSON.stringify(activeContinentRemadeHaptics), "Continent Remade haptic subset drifted");
  ...
  return {
    ...
    id: "continent-remade-responsive-audio-v1",
    ...
    responsiveProgram: {
      ...
      approachTimelineID: "continent-remade-approach-v1",
      interactionBeds: [
        {
          phase: "waiting",
          timelineID: "continent-remade-waiting-bed-v1",
          layerStates: layerStates("waiting"),
        },
        {
          phase: "engaged",
          timelineID: "continent-remade-engaged-bed-v1",
          layerStates: layerStates("engaged"),
        },
        {
          phase: "resistance",
          timelineID: "continent-remade-resistance-bed-v1",
          layerStates: layerStates("resistance"),
        },
      ],
      consequenceTimelineID: "continent-remade-consequence-v1",
      exitPolicy: {
        kind: "bounded-fade",
    ...
    hapticProgram: {
      ...
      activeContinentRemadeSemantics: activeContinentRemadeHaptics,
...
export async function renderContinentRemadeResponsiveAudio({ verifyReproducibility = true } = {}) {
  await rm(cacheRoot, { recursive: true, force: true });
  const first = await renderAll(cacheRoot);
  let reproducibility = "NOT_RUN";
  if (verifyReproducibility) {
    const temporary = await mkdtemp(path.join(os.tmpdir(), "long-west-continent-remade-responsive-"));
    try {
  ...
  const receipt = {
    schemaVersion: 1,
    id: "continent-remade-responsive-audio-render-v1",
    status: "PROVISIONAL_NON_SHIPPING",
    shippingState: "PROHIBITED",
  ...
  await validateContinentRemadeResponsiveAudio();
...
function validateWorkObject(work) {
  ...
  assert(work.id === "continent-remade-responsive-audio-v1", "work object ID drifted");
  ...
  assert(work.scope.beatID === "beat-first-farmers-continent-remade", "work object beat scope drifted");
  assert(work.scope.interactionID === "interaction-first-farmers-a-continent-remade", "work object interaction scope drifted");
  ...
  assert(work.responsiveProgram.approachTimelineID === "continent-remade-approach-v1", "approach timeline drifted");
  assert(work.responsiveProgram.consequenceTimelineID === "continent-remade-consequence-v1", "consequence timeline drifted");
  ...
  assert(JSON.stringify(work.hapticProgram.activeContinentRemadeSemantics) === JSON.stringify(activeContinentRemadeHaptics), "Continent Remade haptic subset drifted");
  assert(JSON.stringify(work.hapticProgram.runtimeBindings) === JSON.stringify([
    { trigger: "transform-drag", semantic: "drag", durableCommitRequired: false },
    { trigger: "causal-threshold-crossed", semantic: "break", durableCommitRequired: false },
    { trigger: "durable-completion", semantic: "seal", durableCommitRequired: true },
  ]), "Continent Remade haptic runtime bindings drifted");
  ...
  ]), "Continent Remade narration slot bindings drifted");
...
export async function validateContinentRemadeResponsiveAudio() {
  ...
  ]).catch(() => fail("Continent Remade responsive work object and receipt are required"));
  const work = JSON.parse(workBytes);
  const receipt = JSON.parse(receiptBytes);
  validateWorkObject(work);
  assert(receipt.id === "continent-remade-responsive-audio-render-v1", "receipt identity drifted");
...
export const continentRemadeResponsiveAudioPaths = Object.freeze({
  repositoryRoot,
  sourceRoot,
  cacheRoot,
  scoreSourcePath,
