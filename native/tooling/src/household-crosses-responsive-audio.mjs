...
const sourceRoot = path.join(audioRoot, "household-crosses-responsive-v1");
const cacheRoot = path.join(audioRoot, "cache/household-crosses-responsive-v1");
...
const workObjectPath = path.join(sourceRoot, "household-crosses-responsive-work-object.json");
...
const activeHouseholdCrossesHaptics = ["contact", "drag", "seal"];
const originAnchorID = "western-anatolia";
const routeStages = [
...
const scoreStates = new Map([
  ["approach", "four-parts-gather-at-waterline"],
  ["waiting", "household-held-at-shore"],
  ["engaged", "four-parts-cross-together"],
  ["resistance", "route-does-not-take"],
  ["consequence", "four-parts-join-as-practice"],
]);
const soundscapeStates = new Map([
  ["approach", "loaded-eastern-shore"],
  ["waiting", "craft-held-in-shallows"],
  ["engaged", "wake-carries-household"],
  ["resistance", "off-route-water-holds"],
  ["consequence", "riverbank-receives-system"],
]);
const silenceSamples = new Map([
  ["approach", { startSample: 2_652_000, durationSamples: 132_000 }],
  ["waiting", { startSample: 710_400, durationSamples: 9_600 }],
  ["engaged", { startSample: 350_400, durationSamples: 7_680 }],
  ["resistance", { startSample: 554_400, durationSamples: 9_600 }],
  ["consequence", { startSample: 744_000, durationSamples: 216_000 }],
]);
const packageRoot = "audio/first-farmers/household-crosses-responsive-v1";
...
function validateScoreSource(source) {
  ...
  assert(JSON.stringify(source.motifs?.map(({ id }) => id)) === JSON.stringify([
    ...
    "animal-mark",
    "tool-mark",
    "learned-practice-mark",
    "resistance-knot",
    "returning-field",
  ]), "scoreSource.motifs: exact crossing vocabulary required");
  ...
  assert(JSON.stringify(source.prohibitedClaims) === JSON.stringify([
    "martial scoring",
    "triumphal scoring",
    "heroic ocean scoring",
    "score tones represented as documented waves or historical evidence",
  ...
  for (const region of source.regions) {
    ...
    assert(JSON.stringify(region.stems.map(({ id, instrument }) => ({
      ...
      { id: "household", program: 42, label: "Cello" },
      { id: "carried-work", program: 15, label: "Dulcimer" },
    ]), `${region.id}: exact crossing stem contract drifted`);
    ...
    assert(region.id === "consequence" ? returningFieldPhrases.length > 0 : returningFieldPhrases.length === 0,
      `${region.id}: returning-field may join only in consequence`);
...
function validateSoundscapeSource(source, scoreSource) {
  ...
  assert(JSON.stringify(source.mixGroups) === JSON.stringify({
    soundscape: ["close-water", "animal-weight", "hull-and-tools"],
    spatialDetail: ["seed-sacks", "carrying-fibre"],
  }), "soundscapeSource.mixGroups drifted");
  assert(JSON.stringify(source.synthesisBoundaries) === JSON.stringify({
    water: {
      algorithm: "close-water-movement-v1",
      audibleClaims: ["close shallows", "wake against a timber body", "close river water"],
      excluded: ["storm", "heroic ocean", "wind propulsion", "specific paddle or oar action", "rowing cadence"],
    },
    animal: {
      algorithm: "herd-ground-movement-v1",
      audibleClaims: ["restrained animal weight shifts", "ground and craft contact", "restrained breath and body movement"],
      excluded: ["species calls", "bells or tack", "gallop rhythm", "stampede", "species identification"],
    },
    craft: {
      algorithm: "distant-material-work-v1",
      audibleClaims: ["timber body contact", "carried tool contact"],
      excluded: ["sail, mast or rigging", "plank construction", "keel or deck", "nails or metal fittings", "propulsive action"],
    },
  }), "soundscapeSource synthesis boundaries drifted");
  assert(JSON.stringify(source.prohibitedClaims) === JSON.stringify([
  ...
  for (const [index, region] of source.regions.entries()) {
    ...
    assert(JSON.stringify(region.layers.map(({ id, material, generator }) => ({
      ...
      { id: "close-water", material: "water", algorithm: "close-water-movement-v1" },
      { id: "animal-weight", material: "herd", algorithm: "herd-ground-movement-v1" },
      { id: "seed-sacks", material: "grain", algorithm: "dry-grain-contact-v1" },
      { id: "carrying-fibre", material: "textile", algorithm: "woven-fibre-friction-v1" },
      { id: "hull-and-tools", material: "work", algorithm: "distant-material-work-v1" },
...
async function productionInputs() {
  return Promise.all([
    inputRecord("household-crosses-responsive-score-source", scoreSourcePath),
    inputRecord("household-crosses-responsive-soundscape-source", soundscapeSourcePath),
    ...
    inputRecord("household-crosses-responsive-renderer", modulePath),
...
export async function validateHouseholdCrossesResponsiveAudioSources() {
  const { scoreSource, soundscapeSource, draft } = await readSources();
  const transformation = findBeat(draft, "beat-first-farmers-household-crosses");
  assert(transformation?.interaction?.grammar === "trace", "Household Crosses source lost its Trace mechanism");
  assert(transformation.interaction.id === "interaction-first-farmers-a-household-crosses", "Household Crosses interaction ID drifted");
  assert(JSON.stringify(transformation.interaction.anchors?.map(({ id }) => id)) === JSON.stringify([
    originAnchorID,
    ...routeStages.map(({ id }) => id),
  ]), "Household Crosses anchor order drifted");
  assert(JSON.stringify(transformation.interaction.haptics) === JSON.stringify(activeHouseholdCrossesHaptics), "Household Crosses haptic subset drifted");
  assert(transformation.interaction.completionEffectID === "effect-first-farmers-a-household-crosses", "Household Crosses completion effect drifted");
  assert(transformation.interaction.causalContract?.persistentTrace === "trace-european-farming-belt", "Household Crosses persistent trace drifted");
  return {
...
async function buildWorkObject({ scoreSource, soundscapeSource, draft, inputs, outputs, metrics }) {
  const transformation = findBeat(draft, "beat-first-farmers-household-crosses");
  const consequence = findBeat(draft, "beat-first-farmers-living-system");
  assert(transformation && consequence, "Household Crosses manuscript beats are missing");
  assert(transformation.interaction?.id === "interaction-first-farmers-a-household-crosses", "Household Crosses interaction ID drifted");
  assert(transformation.interaction.grammar === "trace", "Household Crosses interaction grammar drifted");
  assert(JSON.stringify(transformation.interaction.haptics) === JSON.stringify(activeHouseholdCrossesHaptics), "Household Crosses haptic subset drifted");
  ...
  return {
    ...
    id: "household-crosses-responsive-audio-v1",
    ...
    responsiveProgram: {
      ...
      approachTimelineID: "household-crosses-approach-v1",
      interactionBeds: [
        {
          phase: "waiting",
      ...
      consequenceTimelineID: "household-crosses-consequence-v1",
      exitPolicy: {
        kind: "bounded-fade",
    ...
    hapticProgram: {
      ...
      activeHouseholdCrossesSemantics: activeHouseholdCrossesHaptics,
      reservedForOtherGrammars: ["resistance", "transfer", "break"],
      runtimeBindings: [
      ...
      destinationPreliminaryContact: "FORBIDDEN",
      resistanceSemantic: "FORBIDDEN",
...
export async function renderHouseholdCrossesResponsiveAudio({ verifyReproducibility = true } = {}) {
  await rm(cacheRoot, { recursive: true, force: true });
  const first = await renderAll(cacheRoot);
  let reproducibility = "NOT_RUN";
  if (verifyReproducibility) {
    const temporary = await mkdtemp(path.join(os.tmpdir(), "long-west-household-crosses-responsive-"));
    try {
  ...
  const receipt = {
    schemaVersion: 1,
    id: "household-crosses-responsive-audio-render-v1",
    status: "PROVISIONAL_NON_SHIPPING",
    shippingState: "PROHIBITED",
  ...
  await validateHouseholdCrossesResponsiveAudio();
...
function validateWorkObject(work) {
  ...
  assert(work.id === "household-crosses-responsive-audio-v1", "work object ID drifted");
  ...
  assert(work.scope.arcID === "first-farmers-arc-01", "work object arc scope drifted");
  assert(work.scope.beatID === "beat-first-farmers-household-crosses", "work object beat scope drifted");
  assert(work.scope.interactionID === "interaction-first-farmers-a-household-crosses", "work object interaction scope drifted");
  ...
  assert(work.responsiveProgram.approachTimelineID === "household-crosses-approach-v1", "approach timeline drifted");
  assert(work.responsiveProgram.consequenceTimelineID === "household-crosses-consequence-v1", "consequence timeline drifted");
  ...
  assert(JSON.stringify(work.causalBinding) === JSON.stringify({
    ...
    audioStageAccumulation: "PHASE_LEVEL_ONLY_VISUAL_REDUCER_OWNS_LATCHED_ANCHORS",
  }), "Household Crosses causal binding drifted");
  ...
  assert(JSON.stringify(work.hapticProgram.activeHouseholdCrossesSemantics) === JSON.stringify(activeHouseholdCrossesHaptics), "Household Crosses haptic subset drifted");
  assert(JSON.stringify(work.hapticProgram.runtimeBindings) === JSON.stringify([
    { trigger: "trace-origin-contact", semantic: "contact", durableCommitRequired: false },
    { trigger: "trace-intermediate-anchor-accepted", semantic: "contact", durableCommitRequired: false },
    { trigger: "trace-viable-movement-throttled", semantic: "drag", durableCommitRequired: false },
    { trigger: "trace-destination-durable-commit", semantic: "seal", durableCommitRequired: true },
  ]), "Household Crosses haptic runtime bindings drifted");
  ...
  assert(work.hapticProgram.destinationPreliminaryContact === "FORBIDDEN", "destination acquired preliminary contact");
  assert(work.hapticProgram.resistanceSemantic === "FORBIDDEN", "Trace acquired a resistance haptic");
  ...
  assert(JSON.stringify(work.narrationSlots.map(({ timelineID, requiredAssetPath }) => ({ timelineID, requiredAssetPath }))) === JSON.stringify([
    {
      timelineID: "household-crosses-approach-v1",
      requiredAssetPath: `${packageRoot}/approach/narration-selected-master-required.wav`,
    },
    {
      timelineID: "household-crosses-consequence-v1",
      requiredAssetPath: `${packageRoot}/consequence/narration-selected-master-required.wav`,
    },
  ]), "Household Crosses narration slot bindings drifted");
...
export async function validateHouseholdCrossesResponsiveAudio() {
  ...
  ]).catch(() => fail("Household Crosses responsive work object and receipt are required"));
...
export const householdCrossesResponsiveAudioPaths = Object.freeze({
  repositoryRoot,
  sourceRoot,
  cacheRoot,
  scoreSourcePath,
