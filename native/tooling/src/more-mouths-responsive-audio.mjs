...
const sourceRoot = path.join(audioRoot, "more-mouths-responsive-v1");
const cacheRoot = path.join(audioRoot, "cache/more-mouths-responsive-v1");
...
const workObjectPath = path.join(sourceRoot, "more-mouths-responsive-work-object.json");
...
const activeMoreMouthsHaptics = ["drag", "break", "seal"];
const packageRoot = "audio/first-farmers/more-mouths-responsive-v1";
...
function validateSoundscapeSource(source, scoreSource) {
  ...
  assert(JSON.stringify(source.mixGroups) === JSON.stringify({
    soundscape: ["settlement-hearths", "clearing-work", "herd-lanes"],
    spatialDetail: ["stored-grain", "household-fibre"],
  }), "soundscapeSource.mixGroups drifted");
  assert(JSON.stringify(source.herdSynthesisBoundary) === JSON.stringify({
    algorithm: "herd-ground-movement-v1",
    audibleClaims: [
      "clustered hoof contacts on earth",
      "low group weight shifts",
      "restrained breath and body movement",
    ],
    excluded: [
      "cattle or sheep calls",
      "bells or tack",
      "gallop rhythm",
      "stampede",
      "species identification",
    ],
  }), "soundscapeSource.herdSynthesisBoundary drifted");
  ...
  for (const [index, region] of source.regions.entries()) {
    ...
    assert(JSON.stringify(region.layers.map(({ id, material, generator }) => ({
      ...
      { id: "stored-grain", material: "grain", algorithm: "dry-grain-contact-v1" },
      { id: "household-fibre", material: "textile", algorithm: "woven-fibre-friction-v1" },
      { id: "clearing-work", material: "work", algorithm: "distant-material-work-v1" },
      { id: "herd-lanes", material: "herd", algorithm: "herd-ground-movement-v1" },
...
async function productionInputs() {
  return Promise.all([
    inputRecord("more-mouths-responsive-score-source", scoreSourcePath),
    inputRecord("more-mouths-responsive-soundscape-source", soundscapeSourcePath),
    ...
    inputRecord("more-mouths-responsive-renderer", modulePath),
...
export async function validateMoreMouthsResponsiveAudioSources() {
  const { scoreSource, soundscapeSource, draft } = await readSources();
  const transformation = findBeat(draft, "beat-first-farmers-more-mouths");
  assert(transformation?.interaction?.grammar === "transform", "More Mouths source lost its Transform mechanism");
  assert(transformation.interaction.id === "interaction-first-farmers-more-mouths-more-land", "More Mouths interaction ID drifted");
  assert(JSON.stringify(transformation.interaction.stages) === JSON.stringify([
    ...
    { id: "herd-lanes-and-daughters", controlID: "settlement-pressure", requiredAmount: 1 },
  ]), "More Mouths stage thresholds drifted");
  assert(transformation.interaction.completionEffectID === "effect-first-farmers-more-mouths-more-land", "More Mouths completion effect drifted");
  assert(transformation.interaction.causalContract?.persistentTrace === "trace-european-farming-belt", "More Mouths persistent trace drifted");
  return {
...
async function buildWorkObject({ scoreSource, soundscapeSource, draft, inputs, outputs, metrics }) {
  const transformation = findBeat(draft, "beat-first-farmers-more-mouths");
  const consequence = findBeat(draft, "beat-first-farmers-growth-breaks");
  assert(transformation && consequence, "More Mouths manuscript beats are missing");
  assert(transformation.interaction?.id === "interaction-first-farmers-more-mouths-more-land", "More Mouths interaction ID drifted");
  assert(transformation.interaction.grammar === "transform", "More Mouths interaction grammar drifted");
  assert(JSON.stringify(transformation.interaction.haptics) === JSON.stringify(activeMoreMouthsHaptics), "More Mouths haptic subset drifted");
  ...
  return {
    ...
    id: "more-mouths-responsive-audio-v1",
    ...
    responsiveProgram: {
      ...
      approachTimelineID: "more-mouths-approach-v1",
      interactionBeds: [
        {
          phase: "waiting",
          timelineID: "more-mouths-waiting-bed-v1",
          layerStates: layerStates("waiting"),
        },
        {
          phase: "engaged",
          timelineID: "more-mouths-engaged-bed-v1",
          layerStates: layerStates("engaged"),
        },
        {
          phase: "resistance",
          timelineID: "more-mouths-resistance-bed-v1",
          layerStates: layerStates("resistance"),
        },
      ],
      consequenceTimelineID: "more-mouths-consequence-v1",
      exitPolicy: {
        kind: "bounded-fade",
    ...
    causalBinding: {
      ...
      stages: transformation.interaction.stages.map(({ id, requiredAmount }) => ({ id, requiredAmount })),
    ...
    hapticProgram: {
      ...
      activeMoreMouthsSemantics: activeMoreMouthsHaptics,
...
export async function renderMoreMouthsResponsiveAudio({ verifyReproducibility = true } = {}) {
  await rm(cacheRoot, { recursive: true, force: true });
  const first = await renderAll(cacheRoot);
  let reproducibility = "NOT_RUN";
  if (verifyReproducibility) {
    const temporary = await mkdtemp(path.join(os.tmpdir(), "long-west-more-mouths-responsive-"));
    try {
  ...
  const receipt = {
    schemaVersion: 1,
    id: "more-mouths-responsive-audio-render-v1",
    status: "PROVISIONAL_NON_SHIPPING",
    shippingState: "PROHIBITED",
  ...
  await validateMoreMouthsResponsiveAudio();
...
function validateWorkObject(work) {
  ...
  assert(work.id === "more-mouths-responsive-audio-v1", "work object ID drifted");
  ...
  assert(work.scope.arcID === "first-farmers-arc-03", "work object arc scope drifted");
  assert(work.scope.beatID === "beat-first-farmers-more-mouths", "work object beat scope drifted");
  assert(work.scope.interactionID === "interaction-first-farmers-more-mouths-more-land", "work object interaction scope drifted");
  ...
  assert(work.responsiveProgram.approachTimelineID === "more-mouths-approach-v1", "approach timeline drifted");
  assert(work.responsiveProgram.consequenceTimelineID === "more-mouths-consequence-v1", "consequence timeline drifted");
  ...
  assert(JSON.stringify(work.causalBinding) === JSON.stringify({
    ...
    audioStageAccumulation: "PHASE_LEVEL_ONLY_UNTIL_RUNTIME_SUPPORTS_LATCHED_THRESHOLD_MIXES",
  }), "More Mouths causal binding drifted");
  ...
  assert(JSON.stringify(work.hapticProgram.activeMoreMouthsSemantics) === JSON.stringify(activeMoreMouthsHaptics), "More Mouths haptic subset drifted");
  assert(JSON.stringify(work.hapticProgram.runtimeBindings) === JSON.stringify([
    { trigger: "transform-drag", semantic: "drag", durableCommitRequired: false },
    { trigger: "causal-threshold-crossed", semantic: "break", durableCommitRequired: false },
    { trigger: "durable-completion", semantic: "seal", durableCommitRequired: true },
  ]), "More Mouths haptic runtime bindings drifted");
  ...
  ]), "More Mouths narration slot bindings drifted");
...
export async function validateMoreMouthsResponsiveAudio() {
  ...
  ]).catch(() => fail("More Mouths responsive work object and receipt are required"));
  const work = JSON.parse(workBytes);
  const receipt = JSON.parse(receiptBytes);
  validateWorkObject(work);
  assert(receipt.id === "more-mouths-responsive-audio-render-v1", "receipt identity drifted");
...
export const moreMouthsResponsiveAudioPaths = Object.freeze({
  repositoryRoot,
  sourceRoot,
  cacheRoot,
  scoreSourcePath,
