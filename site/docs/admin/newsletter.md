...
EUROPA uses Brevo for consented chapter-update emails. The site posts its native
...
2. Create a list named `EUROPA · Chapter updates`.
3. Keep the data model minimal: collect only the required email address. Do not
   ...
   records. Use Brevo's standard `EMAIL` field, the custom Boolean
   `CHAPTER_UPDATES_CONSENT` for the form's required consent checkbox, and
   Brevo's automatically maintained `DOUBLE_OPT-IN` status.
...
5. In the setup/design step, turn on **Enable GDPR fields** so Brevo adds both
   its GDPR field and declaration. The consent checkbox must be unchecked by
   default and link to EUROPA's published privacy notice. Use the same consent
   wording as the site: “I consent to Bård Standal using my email address only
   to notify me when new EUROPA chapters are published. I can unsubscribe at any
   time.”
...
7. Customize the double-opt-in email and messages in English. Redirect a
   ...
   message before a chapter notice.
...
- Do not create a welcome or nurture sequence. The list exists only to announce
  substantial new chapters that match the original consent.
...
- In **Settings → Campaigns → Default settings → Tracking & reports**, activate
  ...
  does not request separate pixel-tracking consent, so chapter notices must not
...
- Use `signup-inline-submitted` for the ending form and
  `signup-prompt-submitted` for the floating prompt. A
...
Before each chapter announcement:

1. Confirm the campaign goes only to the chapter-update list and excludes
